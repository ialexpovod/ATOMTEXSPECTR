...
class PlottingERROR(Exception):
    '''
    Общие исключения для айла plottingg.py
    '''
    pass


class PlotSpectrum:  # PlotSpectrum(object) без родительского класса. Не наследуется ни начем
    '''
    ...
    def __init__(self, spectrum, *args, **kwargs):
        ...
        if hasattr(args, "__len__") and len(args) in [0, 1]:
            ...
            raise PlottingERROR("Неправильное количество позиционных аргументов!")
        # d = {'a' :1, 'c' :2}
    ...
    def xmode(self, mode):
        ...
        if mode is None:
            if self.spectrum.is_calibrated:  # Если есть калибровка по энергии, то
                self._xmode = "energy"
            else:
                self._xmode = "channel"
        else:
            if mode.lower() in ("kev", "energy", "mev", "e"):
                if not self.spectrum.is_calibrated:  # Если False (нет калибровок)...
                    raise PlottingERROR(  # Исключение
                ...
                raise PlottingERROR(f"Неизвестный формат данных для оси: {mode}")

        # Тогда, зададим _xedges и _xlabel соновываясь на _xmode
        xedges, xlabel = self.spectrum.parse_xmode(self._xmode)
        self._xedges = xedges
        # Название осей
    ...
    def ymode(self, mode):
        ...
        if mode is None:
            ...
            if self.spectrum._counts is None:
                raise PlottingERROR("У спектра есть отсчеты в каналах, которые не заданы.")
            self._ymode = "counts"
        elif mode.lower() == "cps":
            self._ymode = "cps"
    ...
    def plot(self, *args, **kwargs):
        ...
        if not hasattr(self.args, "__len__") or not len(self.args) in [0, 1]:
            raise PlottingERROR("Неверное число позиционных аргументов!")
    ...
    def errorbar(self, **kwargs):
        ...
        if not hasattr(self.fmt, "__len__") or len(self.fmt) != 1:
            raise PlottingERROR("Неверное число заданных аргументов args")
